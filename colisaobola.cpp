...
bool carregarTextura(){
    bool validez = true;
    imgBola = IMG_Load("../colisao_de_Bolas/sprites/bola.png");
    if (imgBola == NULL){
...
void gerarBolas(Bola* bolas,int n){
    for (int i = 0; i < n; i++){
        ...
        bolas[i].velX = rand() % (/* %(velocidade maxima) + (-velocidade mínima)*/ 5) - 2;
        bolas[i].velY = rand() % (/* %(velocidade maxima) + (-velocidade mínima)*/ 5) - 2;
    }
